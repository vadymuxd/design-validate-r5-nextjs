-- Methods Table Backfill Script
-- This script adds new columns to methods table, backfills data, and creates the scoring function

-- Step 1: Add new columns to methods table (simplified - only what we actually need)
ALTER TABLE methods 
...
-- Step 2: Update descriptions for each method
...
UPDATE methods SET description = 'Examining user-generated data stored in databases to validate design decisions and understand behavior patterns. This method focuses on analyzing structured data such as user selections, purchases, feature interactions, and form inputs. Teams create dashboards and reports to visualize user behavior trends, helping them validate design hypotheses and identify opportunities for product improvement based on actual usage data.' WHERE slug = 'user-data-intelligence';
...
-- Step 3: Create database function to calculate net scores in real-time
...
-- Step 4: Test the function and verify results
SELECT * FROM get_methods_with_scores();

-- Step 5: Show summary stats
SELECT 
    COUNT(*) as total_methods,
    SUM(net_score) as total_net_score,
    AVG(net_score) as avg_net_score_per_method,
    MAX(net_score) as highest_scoring_method,
    MIN(net_score) as lowest_scoring_method
