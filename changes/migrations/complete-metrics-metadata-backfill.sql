-- Complete Metrics Metadata Backfill
-- This script sets all metadata fields for proper categorization across all views
-- Date: July 21, 2025

BEGIN;

-- ====================================================================
-- CLEAR ALL EXISTING METADATA ARRAYS
-- ====================================================================
...
WHERE slug IN (
  ...
  'path-efficiency-score', 'single-usability-metric', 'user-engagement-score', 'workflow-efficiency-score'
);
...
WHERE slug IN (
  ...
  'usability-metric-for-user-experience', 'workflow-efficiency-score'
);
...
WHERE slug IN (
  ...
  'user-satisfaction', 'aesthetic-appeal'
);
...
WHERE slug IN (
  ...
  'success-rate-first-click', 'confusion-rate'
);
...
WHERE slug IN (
  ...
  'system-usability-scale', 'usability-metric-for-user-experience', 'workflow-efficiency-score'
);
...
-- Sample verification for Awareness stage (should show 7 metrics)
SELECT 
    slug,
    name,
    metadata->'user_journey_stage' as user_journey_stage
FROM metrics 
WHERE metadata->'user_journey_stage' ? 'Awareness'
ORDER BY slug;
