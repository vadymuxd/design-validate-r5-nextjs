-- ============================================================================
-- MIGRATION: Create "All in" View for Tools Page  
-- Date: August 28, 2025
-- Purpose: Add universal pro/con text to tools table and create aggregated 
--          "All in" view that shows all tools ranked by total net score
-- ============================================================================
...
-- ============================================================================
-- STEP 3: Create virtual "all-in" method entry (with auto-generated ID)
-- ============================================================================

-- Create a temporary table to store the all-in method ID
CREATE TEMP TABLE all_in_method_id (method_id INTEGER);

-- Insert the "All in" method and capture its ID
DO $$
DECLARE
    all_in_id INTEGER;
BEGIN
    -- Check if 'all-in' method already exists
    SELECT id INTO all_in_id FROM methods WHERE slug = 'all-in';
    
    IF all_in_id IS NULL THEN
        -- Insert new method and get the auto-generated ID
        INSERT INTO methods (collection_id, name, slug, description, current_upvotes, current_downvotes, initial_score, metadata)
        VALUES (1, 'All in', 'all-in', 'Aggregated view of all tools across all methods', 0, 0, 0, '{"view_type":"aggregated","description":"Shows all tools ranked by total net score across all methods"}')
        RETURNING id INTO all_in_id;
    END IF;
    
    -- Store the ID for use in subsequent steps
    INSERT INTO all_in_method_id (method_id) VALUES (all_in_id);
END $$;

-- ============================================================================
-- STEP 4: Create "all-in" leaderboard entries for all tools
-- ============================================================================

-- Create "all-in" leaderboard entries for all existing tools (using the auto-generated method_id)
INSERT INTO tools_leaderboard (tool_id, method_id, initial_upvotes, initial_downvotes, current_upvotes, current_downvotes)
SELECT DISTINCT t.id, aim.method_id, 0, 0, 0, 0
FROM tools t
CROSS JOIN all_in_method_id aim
WHERE NOT EXISTS (
  SELECT 1 FROM tools_leaderboard tl 
  WHERE tl.tool_id = t.id AND tl.method_id = aim.method_id
);

-- ============================================================================
-- STEP 5: Calculate aggregated scores for "all-in" view
-- This aggregates all votes across all methods for each tool
-- ============================================================================

-- Update the "all-in" leaderboard entries with aggregated scores
UPDATE tools_leaderboard 
SET 
  initial_upvotes = (
    SELECT COALESCE(SUM(tl.initial_upvotes + tl.current_upvotes), 0)
    FROM tools_leaderboard tl 
    CROSS JOIN all_in_method_id aim
    WHERE tl.tool_id = tools_leaderboard.tool_id 
    AND tl.method_id != aim.method_id
  ),
  initial_downvotes = (
    SELECT COALESCE(SUM(tl.initial_downvotes + tl.current_downvotes), 0)
    FROM tools_leaderboard tl 
    CROSS JOIN all_in_method_id aim
    WHERE tl.tool_id = tools_leaderboard.tool_id 
    AND tl.method_id != aim.method_id
  ),
  current_upvotes = 0,  -- Reset current votes for "all-in" view
  current_downvotes = 0 -- Will track direct votes on "all-in" view
FROM all_in_method_id aim
WHERE tools_leaderboard.method_id = aim.method_id;

-- ============================================================================
-- STEP 6: Create pros/cons entries for "all-in" view using universal text
-- ============================================================================

-- Insert universal pros/cons for all tools in the "all-in" view (using method_id = 999)
INSERT INTO tool_pros_and_cons (tool_id, method_id, pro_text, con_text)
SELECT t.id, 999, t.pro_text, t.con_text
FROM tools t
WHERE t.pro_text IS NOT NULL 
  AND t.con_text IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM tool_pros_and_cons tpc 
    WHERE tpc.tool_id = t.id AND tpc.method_id = 999
  );
...
SELECT id, name, slug, description FROM methods WHERE id = 999;

-- Check that "all-in" leaderboard entries were created
SELECT COUNT(*) as total_all_in_entries 
FROM tools_leaderboard 
WHERE method_id = 999;

-- Check aggregated scores for top 5 tools in "all-in" view
SELECT 
    t.name,
    tl.initial_upvotes,
    tl.initial_downvotes,
    (tl.initial_upvotes - tl.initial_downvotes) as net_score,
    tpc.pro_text IS NOT NULL as has_universal_pros,
    tpc.con_text IS NOT NULL as has_universal_cons
FROM tools_leaderboard tl
JOIN tools t ON tl.tool_id = t.id
LEFT JOIN tool_pros_and_cons tpc ON tl.tool_id = tpc.tool_id AND tpc.method_id = 999
WHERE tl.method_id = 999
...
-- ============================================================================
-- 1. After running this migration, update your frontend to support "all-in" view
-- 2. Modify /api/tools route to handle method_slug="all-in" 
-- 3. Update voting logic to handle votes for method_id = 999
-- 4. Create aggregation script that runs periodically to update "all-in" scores
-- 5. Set "All in" as the default view in the frontend
-- 6. The universal pro/con text can be edited manually if needed via database
-- 7. Future votes on "all-in" view will be stored with method_id = 999
-- 8. Aggregation should be recalculated when new method-specific votes come in
-- 9. IMPORTANT: Use method_id = 999 for all "All in" view references in code
-- ============================================================================
